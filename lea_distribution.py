...
MODEL_FILES = {
    "Gemma-3-27B": "./results/LEA/independence_results_entire_x_ideal_theta_ideal_y_gemma.json",
    "Mistral-24B": "./results/LEA/independence_results_entire_x_ideal_theta_ideal_y_mistral.json",
    "Llama3.2-3B": "./results/LEA/independence_results_entire_x_ideal_theta_ideal_y_llama.json",
}
...
rows, cols = 1, 3
fig, axes = plt.subplots(rows, cols, figsize=(20, 5), sharey=True)
axes = axes.flatten()
...
for ax_i, model in enumerate(MODEL_FILES):
        ...
        for j, bar in enumerate(bars):
            if vals[j] > 0:
                ...
                ax.text(
                    ...
                    fontsize=13, color=txt_col, weight="bold", zorder=3
                )
    ...
    ax.set_title(model)
    ax.set_xticks(x)
    ax.set_xticklabels([str(yr) for yr in range(2025, 2025 - len(x), -1)], fontsize=14)
    ax.invert_xaxis()                        # years go from 2016 to 2025
    ax.set_xlabel("Years (50 CVEs per year)", fontsize=18)
    ax.grid(axis="y", zorder=0)
    if ax_i % cols == 0:
        ax.set_ylabel("LEA (Mean %)",fontsize=18)
...
handles = [
    Patch(facecolor=PALETTE["xy=True,x?y=True"],  edgecolor=PALETTE["xy=True,x?y=True"], label=r"$A^{fnd}$"),
    Patch(facecolor=PALETTE["xy=True,x?y=False"], edgecolor=PALETTE["xy=True,x?y=False"], label="$A^{rag}$"),
    Patch(facecolor="none", edgecolor=PALETTE["xy=False,x?y=False"], hatch="//", label="$A^{q}$"),
]
fig.legend(
    handles=handles,
    labels=[h.get_label() for h in handles],   # << force these labels
    ncol=3, loc="upper center",
    bbox_to_anchor=(0.5, 1.03), frameon=True, fontsize= 15
)
